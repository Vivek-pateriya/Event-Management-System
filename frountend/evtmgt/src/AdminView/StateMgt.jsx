...
function StateMgt() {
  ...
  const handleAddnewButton = () => {
    axios
      .get('http://localhost:5050/state/getall/')
      .then((res) => {
  ...
  const handleSaveButton = () => {
    if (stid == "" || stid == undefined || stname == "" || stname == undefined || status == "" || status == undefined) {
      ...
      axios.get("http://localhost:5050/state/searchbyname/" + stname).then((res) => {
        if (res.data.stname != undefined) {
          ...
          axios
            .post('http://localhost:5050/state/save/', obj)
            .then((res) => {
              alert(res.data);
              setStid('');
              setStname('');
  ...
  const handleShowbutton = () => {
    axios.get("http://localhost:5050/state/getall/").then((res) => {
      setSlist(res.data);
  ...
  const handleToggle = (stateId, currentState) => {
    ...
    axios.put("http://localhost:5050/state/toggle", data)
      .then((res) => {
        alert(res.data);
        handleShowbutton();
      })
  ...
  const handleSearchbutton = () => {
    if (stid != undefined && stid != "") {
      axios.get("http://localhost:5050/state/search/" + stid).then((res) => {
    ...
    if (!stid || !stname || !status) {
      ...
      axios.get('http://localhost:5050/state/searchbyname/' + stname).then((res) => {
        if (res.data.stname !== undefined) {
          ...
          console.log(obj);
          axios.put('http://localhost:5050/state/update', obj).then((res) => {
