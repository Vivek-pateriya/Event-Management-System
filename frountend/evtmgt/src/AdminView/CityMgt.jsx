...
function Citymgt() {
  ...
  useEffect(() => {
    axios.get('http://localhost:5050/state/show')
      .then((res) => {
        setStList(res.data);
      })
      .catch(err => {
  ...
  const handleAddnewButton = () => {
    axios.get("http://localhost:5050/city/getall").then((res) => {
  ...
  const handleSaveButton = () => {
    if (ctid === "" || ctid === undefined || ctname === "" || ctname === undefined || stid === "" || stid === undefined || status === '0') {
      ...
      axios.post("http://localhost:5050/city/save", obj).then((res) => {
  ...
  const handleSearchButton = () => {
    if (ctid != undefined && ctid != "") {
      axios.get("http://localhost:5050/city/search/" + ctid).then((res) => {
    ...
    if (ctname !== undefined && ctname != "") {
      axios.get("http://localhost:5050/city/searchbyname/" + ctname).then((res) => {
        if (res.data.stid != undefined) {
          setCtid(res.data.ctid);
          setctName(res.data.ctname);
          setStid(res.data.stid);
  ...
  const handleUpdateButton = () => {
    if (stid === "" || stid === undefined || ctname === undefined || status === "" || status === undefined) {
      ...
      axios.put("http://localhost:5050/city/update/", obj).then((res) => {
        alert(res.data);
        setCtid("");
        setctName("");
        setStid("");
  ...
  const handleDeleteButton = () => {
    if (ctid !== undefined && ctid != "") {
      axios.delete("http://localhost:5050/city/delete/" + ctid).then((res) => {
        alert(res.data);
      }).catch((err) => {
        alert(err.message);
      })
  ...
  const handleToggle = (cityId, currentState) => {
    ...
    axios.put("http://localhost:5050/city/toggle", data)
      .then((res) => {
        alert(res.data);
