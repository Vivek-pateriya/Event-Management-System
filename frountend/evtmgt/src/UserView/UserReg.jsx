...
function UserReg() {
  ...
  useEffect(() => {
    ...
    axios
      .get("http://localhost:5050/state/show/")
      .then((res) => {
        setstList(res.data);
      })
      .catch((err) => {
  ...
  const handleRegsistration = async () => {
    ...
    const res = await fetch("http://localhost:5050/user/saveUserimage/", {
      method: "POST",
      body: formData,
    });
    if (res) {
    ...
    axios
      .post("http://localhost:5050/user/regsiter/", obj)
      .then((res) => {
      ...
      <table>
        <tbody>
          ...
          <tr>
            <td>Password</td>
            <td>
              <input type="password" value={CUserPass} onChange={handleUserP} />
            </td>
          </tr>
          <tr>
            <td>Full Name</td>
